...
1. **Anthropic OAuth configured as primary LLM** — The `ANTHROPIC_OAUTH_TOKEN` is set in the `moltbot-secrets` K8s Secret with a valid Anthropic OAuth token, and Moltbot routes all conversations to Claude Opus 4.5 via Anthropic OAuth (FR155).
...
4. **Automatic fallback on Anthropic unavailability** — When the Anthropic API becomes unavailable, Moltbot automatically falls back to LiteLLM proxy within 5 seconds (FR156, NFR88). The three-tier LiteLLM fallback chain activates (vLLM GPU -> Ollama CPU -> OpenAI cloud). The user can identify the fallback provider (FR157).
...
6. **OAuth credential management via control UI** — Via the gateway control UI at `moltbot.home.jetzinger.com`, the operator can view token status and trigger manual refresh if needed (FR158, NFR94).
...
**What Exists:**
- `applications/moltbot/secret.yaml` — Contains `ANTHROPIC_OAUTH_TOKEN` (empty placeholder) and `LITELLM_FALLBACK_URL` (correct value)
- `applications/moltbot/deployment.yaml` — Injects secrets via `envFrom.secretRef`, NFS mount at `/home/node/.moltbot`
- `applications/moltbot/ingressroute.yaml` — Control UI ingress configured
- `applications/litellm/` — Full LiteLLM deployment (9 models: vllm-qwen, vllm-r1, ollama-qwen, openai-gpt4o, groq, gemini, mistral)
- `/home/node/.moltbot/moltbot.json` — Existing config with `trustedProxies` from Story 21.2

**What's Missing:**
- Real OAuth token value in K8s secret
- LLM provider configuration in `moltbot.json`
...
- **Secrets:** All credentials stored as K8s Secrets per NFR91. `ANTHROPIC_OAUTH_TOKEN` and `LITELLM_FALLBACK_URL` already exist in `moltbot-secrets`
- **Config persistence:** `moltbot.json` at `/home/node/.moltbot/moltbot.json` on NFS PVC (10Gi)
...
- `applications/moltbot/secret.yaml` — Already contains `ANTHROPIC_OAUTH_TOKEN` (empty placeholder) and `LITELLM_FALLBACK_URL` (set to correct URL). Only needs the OAuth token populated via `kubectl patch`.
- `applications/moltbot/deployment.yaml` — Injects secrets via `envFrom.secretRef`. No changes expected.
- `/home/node/.moltbot/moltbot.json` (on NFS) — Gateway config file where LLM provider settings are configured. Already exists with `trustedProxies` from Story 21.2.
...
- Control UI confirmed accessible at `https://moltbot.home.jetzinger.com` (0.81s load time)
...
- `applications/moltbot/secret.yaml` needs real credential (NOT committed to git)
- `moltbot.json` on NFS may need LLM provider configuration updates
...
- [Source: docs/implementation-artifacts/21-1-deploy-moltbot-gateway-with-nfs-persistence.md - Previous story]
...
- Pod crashed with invalid `moltbot.json` config — unknown keys `token`, `apiKey`, `fallbackStrategy`, root-level `providers`. Fixed by using correct schema: `models.providers` with required `models` array, auth profiles without inline tokens.
...
- Token patched into `moltbot-secrets` K8s Secret via `kubectl patch` (base64-encoded, not in git)
- `moltbot.json` configured with: Anthropic OAuth auth profile, LiteLLM as `openai-completions` provider with 3 fallback models (vllm-qwen, ollama-qwen, openai-gpt4o), primary model `anthropic/claude-opus-4-5`, fallback `litellm/vllm-qwen`
- Auth credentials written to `/home/node/.moltbot/agents/main/agent/auth-profiles.json`
...
- `applications/moltbot/OAUTH-SETUP.md` — NEW: tutorial for OAuth token setup and renewal
- `applications/moltbot/secret.yaml` — no git changes (token patched at runtime only via `kubectl patch`)
- NFS: `/home/node/.moltbot/moltbot.json` — updated with auth profiles, LiteLLM provider, model config
- NFS: `/home/node/.moltbot/agents/main/agent/auth-profiles.json` — created with Anthropic OAuth credentials
- NFS: `/home/node/.moltbot/agents/main/agent/models.json` — auto-generated by gateway from config
