...
1. **HTTPS IngressRoute configured** — The gateway control UI is accessible at `https://openclaw.home.jetzinger.com` with a valid TLS certificate issued by Let's Encrypt via cert-manager.
...
6. **HTTP-to-HTTPS redirect** — HTTP requests to `openclaw.home.jetzinger.com` are redirected to HTTPS.

## Tasks / Subtasks

- [x] Task 1: Create cert-manager Certificate resource (AC: #1)
  - [x] 1.1 Create `applications/openclaw/ingressroute.yaml` with Certificate for `openclaw.home.jetzinger.com`
  - [x] 1.2 Reference `letsencrypt-prod` ClusterIssuer
  - [x] 1.3 Set `secretName: openclaw-tls`
...
- [x] Task 2: Create Traefik IngressRoute for HTTPS (AC: #1, #3)
  ...
  - [x] 2.2 Match `Host(\`openclaw.home.jetzinger.com\`)`
  - [x] 2.3 Route to `openclaw` service on port 18789 (gateway port from Story 21.1)
  - [x] 2.4 Reference TLS secret `openclaw-tls`
...
- [x] Task 4: Configure DNS rewrite (AC: #1, #3)
  - [x] 4.1 Add `openclaw.home.jetzinger.com` to NextDNS rewrites pointing to MetalLB IP
...
- [x] Task 5: Apply and validate (AC: #1, #2, #3, #4, #5)
  ...
  - [x] 5.4 Access `https://openclaw.home.jetzinger.com` via Tailscale and confirm UI loads - 200 OK
  ...
  - [x] 5.8 Verify HTTP redirect works (`http://openclaw.home.jetzinger.com` → HTTPS) - confirmed 308 redirect
...
**What Exists:**
- `applications/openclaw/` directory with `deployment.yaml`, `service.yaml`, `pvc.yaml`, `secret.yaml` (Story 21.1)
...
**What's Missing:**
- `applications/openclaw/ingressroute.yaml` — needs creation
- No Certificate resource for `openclaw.home.jetzinger.com`
- No IngressRoute resources for openclaw
...
- **Domain:** `openclaw.home.jetzinger.com`
...
- The rewrite for `openclaw.home.jetzinger.com` may already be covered by a wildcard, or may need explicit addition
...
**Rebrand:** The project was renamed Clawdbot → OpenClaw → OpenClaw. As of 2026.1.29:
- npm package: `openclaw`
- Docker Hub image: `openclaw/openclaw` (only tags: `latest` and `2026.1.24`)
- Config paths migrated: `~/.openclaw/openclaw.json` → `~/.openclaw/openclaw.json`
- Env var `CLAWDBOT_GATEWAY_TOKEN` still works (compatibility shim)

**Custom Docker image required:** The official `openclaw/openclaw:latest` Docker image is missing the `extensions/` directory containing bundled plugins (including `memory-core`). Without it, the gateway crashes with `plugins.slots.memory: plugin not found: memory-core`. The fix is to build from source at `https://github.com/openclaw/openclaw` tag `v2026.1.29`, which includes the full `extensions/` directory. The custom image is loaded onto `k3s-worker-01` via `docker save | ssh k3s-worker-01 'sudo ctr -n k8s.io images import -'` and referenced with `imagePullPolicy: Never`.

**Current image:** `docker.io/library/openclaw:2026.1.29` (built from source, ~5GB)

**Entry point:** The Dockerfile default CMD is `node dist/index.js`. The `dist/entry.js` also works. The `openclaw.mjs` / `openclaw.mjs` entry point is for local npm installs only and does not exist in the Docker image.
...
- Old `.openclaw` subPath retained on PVC for reference but no longer mounted
...
- **This story creates:** `applications/openclaw/ingressroute.yaml` (single file with Certificate + 2 IngressRoutes)
...
- [Source: docs/planning-artifacts/architecture.md - OpenClaw Networking Architecture (line ~1449)]
...
- [Source: docs/implementation-artifacts/21-1-deploy-openclaw-gateway-with-nfs-persistence.md - Previous story learnings]
...
- ✅ DNS already configured: `openclaw.home.jetzinger.com` → `192.168.2.100` (Traefik LB)
...
- 📝 Research notes: Used MCP Exa tools to understand openclaw gateway authentication architecture - gateway token vs. device pairing vs. network security layers

**Upgrade to OpenClaw 2026.1.29 (2026-01-31):**
- ✅ Upgraded from `openclaw/openclaw:2026.1.24` to custom-built `openclaw:2026.1.29`
...
- ✅ Updated volume mounts from `.openclaw` (subPath `openclaw`) to `.openclaw` (subPath `openclaw`) to match rebrand
- ✅ Migrated PVC data: copied `openclaw/` subPath contents to `openclaw/` subPath, renamed `openclaw.json` → `openclaw.json`
...
- 📝 Entry point: `dist/entry.js` works in Docker; `openclaw.mjs`/`openclaw.mjs` only exists in npm installs
...
- 2026-01-31: 🔧 Updated volume mounts .openclaw → .openclaw, migrated PVC data
...
- 2026-01-30: 🔍 Used MCP Exa tools to research openclaw gateway auth architecture
...
**Created:**
- `applications/openclaw/ingressroute.yaml` - Certificate + HTTPS IngressRoute + HTTP redirect IngressRoute

**Modified:**
- `applications/openclaw/deployment.yaml` - Image → `docker.io/library/openclaw:2026.1.29`, pullPolicy → `Never`, volume mounts → `.openclaw`, removed `MOLT_GATEWAY_TRUSTED_PROXIES` env var
- `applications/openclaw/secret.yaml` - Re-enabled `CLAWDBOT_GATEWAY_TOKEN` with secure 32-byte token (gitignored)
...
- PVC `openclaw-data` subPath `openclaw/openclaw.json` - Gateway config with trustedProxies, allowInsecureAuth, memory-core
- PVC `openclaw-data` subPath `openclaw/devices/` - Device pairing state
